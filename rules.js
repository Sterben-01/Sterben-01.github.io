...
function begin_play(){
	var playername_1 = document.getElementById("player1_id");
	var playername_2 = document.getElementById("player2_id");
	if(isEmpty(playername_1.value) || isEmpty(playername_2.value)){
		alert("Please enter the player name for both.");
		return;
	}
	if(game_started() == true){
		...
		playername_1.disabled = true;
		playername_2.disabled = true;
		var turn_status = document.getElementById("turn_info");
		playername_1.value = playername_1.value + " (X)";
		playername_2.value = playername_2.value + " (O)";
		turn_status.innerHTML = "Turn for : <b>X</b>";
		this.started = true;
	}
}
...
function reset_play(){
	if(game_started() == false)
	{
		...
		var playername_2 = document.getElementById("player2_id");
		var turn_status = document.getElementById("turn_info");
		var mov_text = document.getElementById("move_text_id");
		playername_1.value = "";
		playername_2.value = "";
		playername_1.disabled = false;
		playername_2.disabled = false;
		mov_text = "";
...
function play() {
	...
	var mov_step = document.getElementById("move_text_id");
	var check_condition = check_move(mov_step.value);
	if(check_condition == true){
		mov_step.value = "";
		var victory_or_not = (check_victory());
		if(victory_or_not == true){
			if (this.turn){
				alert("Winner is : X");
				reset_play();
			}
...
function check_move(move_step){
	var input_movestep = document.getElementById("move_text_id");
	if(!table_ids.includes(input_movestep.value)){
		alert("Invalid input. Please try again.");
		return false;
	}
	var value_cell = document.getElementById(input_movestep.value);
	var checklist = ["X", "O"];
	if(checklist.includes(value_cell.innerText)){
		alert("You can not move to an already-occupied cell!");
		return false;
	}
	var mov_index = -1;
	for (i = 0; i < this.table_ids.length; i++) {
		if (move_step == this.table_ids[i]) {
			mov_index = i;
		}
	}
	if (this.turn) {
		document.getElementById(move_step).innerHTML = "<b>X</b>";
	}
	else {
		document.getElementById(move_step).innerHTML = "<b>O</b>";
	}

	this.board_state[mov_index] = this.turn;
	return true;
